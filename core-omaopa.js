...
import {
  ...
  runTransaction, collection, getDocs, getCountFromServer, getAggregateFromServer, sum, query, orderBy, where, limit, startAfter, documentId, arrayUnion
...
async function membersByMonth(monthsBack, outletNames){
  ...
  for(let i=monthsBack-1;i>=0;i--){
    ...
    let count=0;
    try{
      let constraints=[where('createdAt','>=',start), where('createdAt','<',end)];
